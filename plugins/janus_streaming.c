...
static void * janus_videoroom_postprocess(void * data)
{
	...
	AVFrame * m_pFrame, * o_pFrame, * sws_frame ; 
	av_register_all() ;
	...
	check_decode_process : 
			...
			int num_bytes = avpicture_get_size(AV_PIX_FMT_YUV420P, 800, 600) ;
			uint8_t* frame2_buffer = compFrame ; 
			avpicture_fill((AVPicture*) sws_frame, frame2_buffer, AV_PIX_FMT_YUV420P, 800, 600) ;
			sws_frame->format = AV_PIX_FMT_YUV420P ; 
			sws_frame->height = 600 ; 
			sws_frame->width = 800 ; 
			sws_scale(resize, m_pFrame->data, m_pFrame->linesize, 0, vp8h, sws_frame->data, sws_frame->linesize) ;
