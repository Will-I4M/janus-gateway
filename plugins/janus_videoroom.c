...
void janus_videoroom_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	if((!video && participant->audio_active) || (video && participant->video_active)) {
		...
		if (participant->video_active && participant->vrc)
		{
		...
		{	
			if((vp8_h[0] == 0x9D) && (vp8_h[1] == 0x01) && (vp8_h[2] == 0x2A))
			{
				...
				JANUS_LOG(LOG_VERB,"Key Frame !!! Start recording %d now : %"SCNu64" filename : %s\n",participant->vrc->key_frame_received,ts_now,participant->vrc->filename) ;
				char sz_ts_now[64] ; 
		...
		{
			janus_mutex_lock(&pp_publishers_mutex) ; 
			...
			janus_mutex_unlock(&pp_publishers_mutex) ; 
		}
...
static void * janus_videoroom_postprocess(void * data)
{
	...
	JANUS_LOG(LOG_INFO, "[%"SCNu64"]	BEGIN POSTPROCESS !!! \n",userdata->participant->user_id) ;
	...
	AVFrame * m_pFrame = NULL, * sws_frame = NULL ; 
	av_register_all() ;
	...
	vStream->codec->width = 800 ;
	vStream->codec->height = 600 ;
	...
	while(!g_atomic_int_get(&stopping) && alive)
	{
		...
		if(!start) // Empty packet list -> wait 
		{
			usleep(2000) ; 
			janus_mutex_lock(&pp_publishers_mutex) ; 
			if(!g_hash_table_lookup(userdata->participant->room->participants, GUINT_TO_POINTER(userdata->participant->user_id)))
				g_hash_table_remove(pp_publishers, GUINT_TO_POINTER(userdata->participant->user_id)) ;		
			if(!g_hash_table_lookup(pp_publishers, GUINT_TO_POINTER(userdata->participant->user_id)))
			{
				g_hash_table_remove(pp_publishers, GUINT_TO_POINTER(userdata->participant->user_id)) ;
				alive = 0 ; 
			}
	...
	process_frame : 
		...
		else
		{
			...
			if(reinit_decoder)
			{
				...
				resize = sws_getContext(vp8w,vp8h, AV_PIX_FMT_YUV420P, 800,600, AV_PIX_FMT_YUV420P, SWS_BICUBIC, NULL, NULL, NULL) ; 
				reinit_decoder = 0 ; 
			}
	...
	check_decode_process : 
			...
			avpicture_fill((AVPicture*) sws_frame, compFrame, AV_PIX_FMT_YUV420P, 800, 600) ;
			sws_frame->format = AV_PIX_FMT_YUV420P ; 
			sws_frame->height = 600 ; 
			sws_frame->width = 800 ; 
			sws_scale(resize, (const uint8_t *const *)(m_pFrame->data), m_pFrame->linesize, 0, vp8h, sws_frame->data, sws_frame->linesize) ;
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(session->participant_type == janus_videoroom_p_type_none) {
			...
			else if(!strcasecmp(request_text, "postprocess")) 
			{
				...
				// add the publisher in pp_publishers hashtable and launch a thread 
